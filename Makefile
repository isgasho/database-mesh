.PHONY: build
build:
	make beauty && mkdir -p bin; go build -o bin/database-mesh proxy/database-mesh.go
beauty:
	gofmt -w .
run:
...
linux:
	GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o bin/database-mesh proxy/database-mesh.go
